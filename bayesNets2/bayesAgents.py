...
def constructBayesNet(gameState):
    ...
    #find the obs variables
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVars.append(OBS_VAR_TEMPLATE % obsPos)

    #set the variabledomains of each variable
    ...
    for OBS_VAR_EL in obsVars:
        variableDomainsDict[OBS_VAR_EL] = OBS_VALS

    #set the edges of each variable
    ...
    for OBS_VAR_EL in obsVars:
        edges.append((FOOD_HOUSE_VAR, OBS_VAR_EL))
        edges.append((GHOST_HOUSE_VAR, OBS_VAR_EL))
...
def fillObsCPT(bayesNet, gameState):
    ...
    HV = [BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL]
    "*** YOUR CODE HERE ***"
    for i, housePos in enumerate(gameState.getPossibleHouses()):
        HOUSE_POS = HV[i]
        for obsPos in gameState.getHouseWalls(housePos):
            obsPos = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsPos], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], variableDomainsDict)
            for FOOD_HOUSE_VAL in HOUSE_VALS:
                for GHOST_HOUSE_VAL in HOUSE_VALS:
                    #if next to foodhouse
                    if HOUSE_POS == FOOD_HOUSE_VAL:
                        obsFactor.setProbability({obsPos: BLUE_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 1.0 - PROB_FOOD_RED)
                        obsFactor.setProbability({obsPos: RED_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, PROB_FOOD_RED )
                        obsFactor.setProbability({obsPos: NO_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 0.0)
                    #if next to a ghosthouse
                    if HOUSE_POS != FOOD_HOUSE_VAL and HOUSE_POS == GHOST_HOUSE_VAL:
                        obsFactor.setProbability({obsPos: BLUE_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 1.0 - PROB_GHOST_RED)
                        obsFactor.setProbability({obsPos: RED_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, PROB_GHOST_RED )
                        obsFactor.setProbability({obsPos: NO_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 0.0)
                    #if next to neither house
                    if HOUSE_POS != FOOD_HOUSE_VAL and HOUSE_POS != GHOST_HOUSE_VAL:
                        obsFactor.setProbability({obsPos: BLUE_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 0.0)
                        obsFactor.setProbability({obsPos: RED_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 0.0)
                        obsFactor.setProbability({obsPos: NO_OBS_VAL, FOOD_HOUSE_VAR:FOOD_HOUSE_VAL, GHOST_HOUSE_VAR:GHOST_HOUSE_VAL}, 1.0)
                bayesNet.setCPT(obsPos, obsFactor)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    inferenceByVariableElimination = inference.inferenceByVariableEliminationWithCallTracking()
    foodHouseFactor = inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)
    probRankings = []

    for HOUSE_VAL in HOUSE_VALS:
        evidence[FOOD_HOUSE_VAR] = HOUSE_VAL
        probRankings.append(foodHouseFactor.getProbability(evidence))
    best_house_val = HOUSE_VALS[probRankings.index(max(probRankings))]
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        inferenceByVariableElimination = inference.inferenceByVariableEliminationWithCallTracking()
        houseGhostFactor = inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)

        evidence[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        probHouseIsTopLeft = houseGhostFactor.getProbability(evidence)

        evidence[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        evidence[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        probHouseIsTopRight = houseGhostFactor.getProbability(evidence)

        rightExpectedValue = probHouseIsTopRight * WON_GAME_REWARD + probHouseIsTopLeft * GHOST_COLLISION_REWARD
        leftExpectedValue = probHouseIsTopLeft * WON_GAME_REWARD + probHouseIsTopRight * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, newEvidence in probsAndOutcomes:
            enterValues = self.computeEnterValues(newEvidence, enterEliminationOrder)
