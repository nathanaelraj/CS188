...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        inputs = []
        for parentNode in node.get_parents():
            inputs.append(self.get_output(parentNode))
    ...
    def backprop(self):
        ...
        for node in reversed(self.nodes):
            ...
            for i, parent in enumerate(node.get_parents()):
                self.gradient[parent] += gradient[i]
    ...
    def step(self, step_size):
        ...
        for variable in self.variables:
            variable.data -= step_size * self.get_gradient(variable)
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.where(inputs[0] > 0, inputs, 0).reshape(inputs[0].shape)
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        x,y = a.shape
        N = float(x*y)
        a_prime = gradient * (1/N * (a-b))
        b_prime = gradient * (1/N * (b-a))
